    ...
    def get(self, request, pk, **kwargs):
        ...
        if Consulta.objects.filter(username=request.user, titulo=libro.titulo).exists() == False:
            for autor in libro.autor.all():
                autor
            for materia in libro.materia.all():
                materia
            busqueda = Consulta(username=request.user, cota=libro.cota, titulo=libro.titulo, autor_nombre=autor.nombre, autor_apellido=autor.apellido, tema=materia,tipo_material="Libro")
            busqueda.save()
