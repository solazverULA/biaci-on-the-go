    ...
    def get(self, request, pk, **kwargs):
        ...
        if Consulta.objects.filter(username=request.user, titulo=tesis.titulo).exists() == False:
            for autor in tesis.autor.all():
                autor
            busqueda = Consulta(username=request.user,cota=tesis.cota,titulo=tesis.titulo,autor_nombre=autor.nombre,autor_apellido=autor.apellido,tema=tesis.area,tipo_material="Tesis")
            busqueda.save()
